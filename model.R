...
# For condos, we exclude characteristic variables and add the two strata
# variables
mod_predictors <- ccao::vars_dict %>%
  filter(var_is_predictor, str_sub(var_name_standard, 1, 5) != "char_") %>%
  pull(var_name_standard) %>%
  unique() %>%
  na.omit() %>%
  c("meta_strata_10", "meta_strata_300")
