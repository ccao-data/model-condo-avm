...
table(assessment_data_pred$test1, assessment_data_pred$meta_strata_1)

assessment_data_pred <- assessment_data_pred %>%
  mutate(
    pred_card_initial_fmv = predict(
      lgbm_final_full_fit,
      new_data = baked_predictors
    )$.pred
  )

final_data_with_predictors <- assessment_data_pred %>%
  bind_cols(baked_predictors)
