...
```{r}
strata <- assessment_data %>%
  filter(meta_modeling_group != "NONLIVABLE") %>%
  select(meta_pin, meta_strata_1, meta_mailed_tot, township = meta_township_name) %>%
  ...
  mutate(meta_strata_1 = as.numeric(meta_strata_1),
         meta_mailed_tot = meta_mailed_tot * 10)
```

### Mailed Values * 10

```{r}
ggplot(strata %>% filter(meta_mailed_tot <= 2000000), aes(x = factor(meta_strata_1), y = meta_mailed_tot)) +
  geom_boxplot() +
  labs(
    title = "Box and Whisker Plot of FMV by Strata",
    subtitle = "Display capped at $2,000,000",
    x = "Strata",
    y = "Predicted Initial FMV"
  ) +
  theme_minimal()
...
```{r}
ggplot(strata %>% filter(meta_sale_price < 2000000), aes(x = factor(meta_strata_1), y = meta_sale_price)) +
  geom_boxplot() +
  labs(
    title = "Box and Whisker Plot of Sale Price by Strata",
    subtitle = "Display capped at $2,000,000",
    x = "Strata",
    y = "Sale Price"
  ) +
...
strata_descriptive <- strata %>%
  group_by(meta_strata_1) %>%
  summarize(
    meta_mailed_tot_min = min(meta_mailed_tot, na.rm = TRUE),
    meta_sale_price_min = min(meta_sale_price, na.rm = TRUE),
    meta_mailed_tot_mean = mean(meta_mailed_tot, na.rm = TRUE),
    meta_sale_price_mean = mean(meta_sale_price, na.rm = TRUE),
    meta_mailed_tot_median = median(meta_mailed_tot, na.rm = TRUE),
    meta_sale_price_median = median(meta_sale_price, na.rm = TRUE),
    meta_mailed_tot_max = max(meta_mailed_tot, na.rm = TRUE),
...
plots <- map(strata_groups, ~ {
  p <- ggplot(.x, aes(x = meta_mailed_tot, y = meta_sale_price, 
                      color = township,
                      text = paste("PIN:", meta_pin, 
                                   "<br>Sale Price:", dollar(meta_sale_price), 
                                   "<br>Assessed Value:", dollar(meta_mailed_tot),
  ...
  ggplotly(p, tooltip = "text")  # Make plot interactive with custom tooltip
})
```
