...
```{r results='asis'}
outcomes_strata_plots <- purrr::map(outcomes_strata_groups, ~ {
  p <- ggplot(.x, aes(
    x = pred_pin_final_fmv,
    y = sale_recent_1_price,
    color = township,
    text = paste(
      "PIN:", meta_pin,
      "<br>Sale Price:", scales::dollar(sale_recent_1_price),
      "<br>Assessed Value:", scales::dollar(pred_pin_final_fmv),
      "<br>Township:", township,
      "<br>Imputed:", imputed
    )
  )) +
    ...
    scale_x_continuous(limits = c(0, 2000000), labels = scales::dollar) +
    scale_y_continuous(limits = c(0, 2000000), labels = scales::dollar) +
    theme_minimal()
...
for (idx in seq_along(outcomes_strata_plots)) {
  tab_id <- paste0("tabset-strata-", idx)
  ...
  nav_tabs <- c(
    ...
    withTags(
      li(
        ...
        role = "presentation",
        a(
          class = ifelse(idx == 1, "nav-link active", "nav-link"),
          id = paste0(tab_id, "-tab"),
          "data-bs-toggle" = "tab",
          "data-bs-target" = paste0("#", tab_id),
          role = "tab",
          "aria-controls" = tab_id,
          "aria-selected" = ifelse(idx == 1, "true", "false"),
          href = "",
          paste("Strata Plot", idx)
        )
      )
    )
  )
  ...
  tab_contents <- c(
    ...
    withTags(
      div(
        id = tab_id,
        class = ifelse(idx == 1, "tab-pane active", "tab-pane"),
        role = "tabpanel",
        "aria-labelledby" = paste0(tab_id, "-tab"),
        outcomes_strata_plots[[idx]]
      )
    )
  )
}
