...
## Strata
Because condo characteristics are sparse, and that condo units have similar characteristics, we group condos into strata. These are both directly calculated, as well as imputed through a KNN process during the ingest stage of modeling.
...
strata <- assessment_pin %>%
  ...
  left_join(
    training_data_distinct %>% select(meta_pin, meta_sale_price),
    by = "meta_pin"
  ) %>%
...
```{r _outcomes_predicted_fmv_imputed, warning=FALSE}
strata %>%
...
strata_descriptive <- strata %>%
  ...
  rename_with(~ sub("^meta_", "", .x)) %>%
  mutate(
    across(where(is.numeric), ~ dollar(round(., 2)))
  )
...
plots <- map(strata_groups, ~ {
  p <- ggplot(.x, aes(
    x = pred_pin_final_fmv, y = meta_sale_price,
    color = township,
    text = paste(
      "PIN:", meta_pin,
    ...
    geom_point(aes(shape = as.factor(imputed))) +
    labs(
      x = "FMV",
      y = "Sale Price",
...
```{r _outcomes_strata_scatterplot, results='asis'}
iwalk(plots, ~ {
  cat("## ", .y, "\n\n")
