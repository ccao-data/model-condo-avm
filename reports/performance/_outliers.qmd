...
y_lim_axis_outlier_breakdown <- price_data %>%
...
This table also isolated the outlier reasons to the first reason that
makes the sale an outlier.

```{r _outliers_type_breakdown_table}
training_data %>%
  filter(sv_is_outlier) %>%
  group_by(meta_year, sv_outlier_reason1) %>%
  summarise(n = n()) %>%
  rename(Year = meta_year) %>%
  pivot_wider(id_cols = Year, names_from = sv_outlier_reason1, values_from = n) %>%
  kable() %>%
...
```
<!-- TODO: Perhaps incorporate all the other types here? -->
