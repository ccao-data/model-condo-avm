...
pv_summary <- pv_final_values %>%
  ...
  summarize(
    ...
    median_model_value = median(lgbm, na.rm = TRUE),
...
pv_final_values %>%
  pivot_longer(c(lgbm, final_value)) %>%
  mutate(
    ratio = value / meta_sale_price,
    name = fct_recode(
      factor(name, levels = c("lgbm", "final_value")),
      "Model Value" = "lgbm",
...
decile_data_ratio <- pv_final_values %>%
  filter(
    ...
    !is.na(lgbm), !is.na(yoy_pct_change)
  ) %>%
  mutate(
    decile = ntile(meta_sale_price, 10),
    across(c(lgbm, final_value), ~ .x / meta_sale_price)
  ) %>%
  group_by(decile) %>%
  ...
  pivot_longer(c(lgbm, final_value), names_to = "name", values_to = "ratio") %>%
  mutate(
    name = fct_recode(
      factor(name, levels = c("final_value", "lgbm")),
      "Model Value" = "lgbm",
      "Final Value" = "final_value"
    )
  )
...
decile_data_yoy <- pv_final_values %>%
  ...
  mutate(
    ...
    across(c(lgbm, final_value), ~ (.x - prior_value) / prior_value)
  ) %>%
  group_by(decile) %>%
  mutate(decile_label = paste(
  ...
  ungroup() %>%
  mutate(decile_label = fct_rev(fct_reorder(factor(decile_label), decile))) %>%
  pivot_longer(c(lgbm, final_value), names_to = "name", values_to = "value") %>%
  mutate(
    name = fct_recode(
      factor(name, levels = c("final_value", "lgbm")),
      "Model Value" = "lgbm",
...
pv_final_values %>%
  filter(!is.infinite(yoy_pct_change) & !is.na(geo_latitude)) %>%
  mutate(
